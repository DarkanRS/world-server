	...
	public static boolean dumpNPC(String pageName, NPCCombatDefinitions originalDef, int combatLevel, boolean folderItUp) {
		try {
			...
			if (multi) {
				...
				cbDef.setLevels(new int[] { higher(atks[closestIdx], atk), higher(strs[closestIdx], str), higher(defs[closestIdx], def), higher(ranges[closestIdx], range), higher(mages[closestIdx], mage) });
				...
				cbDef.setLevels(new int[] { atk, str, def, range, mage });
				saveDef(pageName + (folderItUp ? (" (" + combatLevel + ")") : ""), cbDef, folderItUp ? pageName.toLowerCase().replace(" ", "_") : null);
			}
			return true;
		} catch (Exception e) {
