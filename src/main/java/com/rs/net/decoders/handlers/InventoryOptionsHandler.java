...
import com.rs.game.player.content.skills.cooking.FruitCuttingD;
...
import com.rs.game.player.content.skills.crafting.GemTipCutting;
import com.rs.game.player.content.skills.crafting.GemTipCutting.GemTips;
import com.rs.game.player.content.skills.dungeoneering.DungeonRewards;
import com.rs.game.player.content.skills.farming.TreeSaplings;
import com.rs.game.player.content.skills.firemaking.Firemaking;
import com.rs.game.player.content.skills.herblore.CoconutCracking;
import com.rs.game.player.content.skills.herblore.HerbCleaning;
import com.rs.game.player.content.skills.herblore.WeaponPoison;
...
import com.rs.game.player.content.skills.smithing.GodSwordCreation;
...
public class InventoryOptionsHandler {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (player.getTreasureTrailsManager().useDig())
					return;
				if (BarrowsController.digIntoGrave(player))
					return;
	...
	public static void handleItemOption1(Player player, final int slotId, final int itemId, Item item) {
		...
		if (item.getId() == 20667) {
			...
			player.getSkills().set(Constants.MAGIC, (int) (level + 6));
			return;
		}
		...
		if (itemId >= 2520 && itemId <= 2526) {
			String[] phrases = new String[] { "Come on Dobbin, we can win the race!", "Hi-ho Silver, and away!", "Neaahhhyyy! Giddy-up horsey!" };
		...
		else if (ItemTeleports.transportationDialogue(player, item))
			return;
		if (itemId == 19967) {
			if (Magic.sendTeleportSpell(player, 7082, 7084, 1229, 1229, 1, 0, new WorldTile(2952, 2933, 0), 4, true, Magic.ITEM_TELEPORT))
	...
	public static boolean handleItemOnItem(Player player, Item used, Item usedWith, int fromSlot, int toSlot) {
		...
		if (GodSwordCreation.handleGodSword(player, usedWithId, usedId)) {
			return true;
		}
		
		if (WeaponPoison.poison(player, used, usedWith, false))
			return true;
		...
		if (Firemaking.isFiremaking(player, used, usedWith))
			return true;
		else if (GemCutting.isCutting(player, used, usedWith))
			return true;
