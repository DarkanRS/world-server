...
//					return;
				...
				// start falador mininig
					...
					player.getDialogueManager().execute(new EnterHouse(), new Object[]{});
				// rock living caverns
				else if (id == 45077) {
					player.lock();
				...
				else if (WildernessController.isDitch(id)) {// wild ditch
...
//						break;
	...
	public static void handleOption2(final Player player, final GameObject object) {
		...
		if (!def.containsOption(1))
			return;
		
		if (PluginManager.handle(new ObjectClickEvent(player, object, ClientPacket.OBJECT_OP2, false)))
			return;
		...
		player.setRouteEvent(new RouteEvent(object, new Runnable() {
			...
			public void run() {
				...
				else if (object.getDefinitions(player).getName().equalsIgnoreCase("furnace") || object.getDefinitions(player).getName().equalsIgnoreCase("clay forge") || object.getDefinitions(player).getName().equalsIgnoreCase("lava furnace"))
	...
	public static void handleOption3(final Player player, final GameObject object) {
		...
		if (!def.containsOption(2))
			return;

		if (PluginManager.handle(new ObjectClickEvent(player, object, ClientPacket.OBJECT_OP3, false)))
			return;
...
//						});
	...
	public static void handleOption4(final Player player, final GameObject object) {
		...
		if (!def.containsOption(3))
			return;

		if (PluginManager.handle(new ObjectClickEvent(player, object, ClientPacket.OBJECT_OP4, false)))
			return;
	...
	public static void handleOption5(final Player player, final GameObject object) {
		...
		if (!def.containsOption(4))
			return;
		
		if (PluginManager.handle(new ObjectClickEvent(player, object, ClientPacket.OBJECT_OP5, false)))
			return;
