...
//    protected final static int
    ...
    public static void findTreasure(Player p) {
        if(p.getQuestManager().getStage(Quest.PIRATES_TREASURE) != GET_TREASURE)
            return;
        if(!p.getQuestManager().getAttribs(Quest.PIRATES_TREASURE).getB(PiratesTreasure.KNOWS_TREASURE_LOC_ATTR))
            return;
        if(Utils.getDistance(p, new WorldTile(2999, 3383, 0)) <= 2) {
            ...
            for(NPC npc : World.getNPCsInRegion(p.getRegionId()))
                if(npc.getId()== HOSTILE_GARDENER)
                    return;
            NPC gardener = World.spawnNPC(HOSTILE_GARDENER, p, -1, false, true);
