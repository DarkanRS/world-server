...
public class HouseController extends Controller {
	...
	public boolean processObjectClick1(final GameObject object) {
		if (object.getDefinitions().getOption(1).equalsIgnoreCase("Pray"))
			return true;
		if (object.getDefinitions().getOption(1).equalsIgnoreCase("Renew-points") && object.getDefinitions().getName().contains("obelisk"))
			return true;
	...
	private void handleCombatRing(GameObject object) {
		...
		case 0:
			if (player.getX() < object.getX()) {
				target = object.transform(1, 0, 0);
				direction = Direction.EAST;
				player.getTemporaryAttributes().put("inBoxingArena", false);
			} else {
				target = object.transform(-1, 0, 0);
				direction = Direction.WEST;
				player.getTemporaryAttributes().put("inBoxingArena", true);
			}
			break;
		case 1:
			if (player.getY() <= object.getY()) {
	...
	public boolean processItemOnObject(GameObject object, Item item) {
		if (object.getId() == HouseConstants.HObject.CLAY_FIREPLACE.getId() || object.getId() == HouseConstants.HObject.STONE_FIREPLACE.getId() || object.getId() == HouseConstants.HObject.MARBLE_FIREPLACE.getId()) {
			...
			return false;
		} else if (Builds.SINK.containsObject(object)) {
	...
	public void process() {
		...
		if (player.getTemporaryAttributes().get("inBoxingArena") != null && (boolean) player.getTemporaryAttributes().get("inBoxingArena"))
			inBoxing = true;
