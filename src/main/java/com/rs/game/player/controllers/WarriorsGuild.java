...
public class WarriorsGuild extends Controller {
	...
	public boolean processObjectClick1(GameObject object) {
		if (object.getId() >= 15624 && object.getId() <= 15630) {
			...
			if (player.getTemporaryAttributes().get("thrown_delay") != null) {
	...
	public boolean processItemOnObject(final GameObject object, final Item item) {
		if (object.getId() == 15621) {
			if (player.getTemporaryAttributes().get("animator_spawned") != null) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					if (ticks == 0)
						...
						player.getTemporaryAttributes().put("animator_spawned", true);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 3) {
					...
					player.getTemporaryAttributes().put("thrown_delay", true);
					...
					player.getTemporaryAttributes().remove("thrown_delay");
					stop();
					return;
				}
			}
