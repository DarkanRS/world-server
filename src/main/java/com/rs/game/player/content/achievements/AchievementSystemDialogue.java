...
public class AchievementSystemDialogue extends Conversation {
	...
	public AchievementSystemDialogue(Player player, int npcId, SetReward reward) {
		...
		if (reward.hasRequirements(player, reward.getItemIds()[0])) {
			...
			start.addNPC(npcId, HeadE.SHAKING_HEAD, "Unfortunately not. The requirements for claiming the first tier have been listed in your chatbox.");
		}
	}
