...
public class Potions {
	...
	public enum Potion {
		...
		PRAYER_POTION(VIAL, new int[] { 2434, 139, 141, 143 }, p -> p.getPrayer().restorePrayer((int) ((int) (Math.floor(p.getSkills().getLevelForXp(Constants.PRAYER) * 2.5) + 70)))),
		PRAYER_FLASK(-1, new int[] { 23243, 23245, 23247, 23249, 23251, 23253 }, p -> p.getPrayer().restorePrayer((int) ((int) (Math.floor(p.getSkills().getLevelForXp(Constants.PRAYER) * 2.5) + 70)))),
		PRAYER_MIX(VIAL, new int[] { 11465, 11467 }, p -> {
			p.getPrayer().restorePrayer((int) ((int) (Math.floor(p.getSkills().getLevelForXp(Constants.PRAYER) * 2.5) + 70)));
			p.heal(30);
		}),
		
		SUPER_RESTORE(VIAL, new int[] { 3024, 3026, 3028, 3030 }, p -> {
			p.getSkills().adjustStat(8, 0.25, false, Utils.range(0, Skills.SIZE-1));
			p.getPrayer().restorePrayer((int) ((int) (p.getSkills().getLevelForXp(Constants.PRAYER) * 0.33 * 10)));
		}),
		...
		SUPER_PRAYER(VIAL, new int[] { 15328, 15329, 15330, 15331 }, p -> p.getPrayer().restorePrayer((int) ((int) (70 + (p.getSkills().getLevelForXp(Constants.PRAYER) * 3.43))))),
		SUPER_PRAYER_FLASK(-1, new int[] { 23525, 23526, 23527, 23528, 23529, 23530 }, p -> p.getPrayer().restorePrayer((int) ((int) (70 + (p.getSkills().getLevelForXp(Constants.PRAYER) * 3.43))))),
		...
		WEAK_REJUVENATION_POTION(17490, 17570, p -> {
			...
			p.getPrayer().restorePrayer((int) ((int) (Math.floor(p.getSkills().getLevelForXp(Constants.PRAYER)) + 50)));
		}),
		REJUVENATION_POTION(17490, 17594, p -> {
			p.getSkills().adjustStat(7, 0.15, false, Constants.SUMMONING);
			p.getPrayer().restorePrayer((int) ((int) (Math.floor(p.getSkills().getLevelForXp(Constants.PRAYER) * 1.5) + 70)));
		}),
		STRONG_REJUVENATION_POTION(17490, 17618, p -> {
			p.getSkills().adjustStat(10, 0.22, false, Constants.SUMMONING);
			p.getPrayer().restorePrayer((int) ((int) (Math.floor(p.getSkills().getLevelForXp(Constants.PRAYER) * 2.2) + 100)));
		}),
		...
		private final void drink(Player player, int itemId, int slot) {
			if (player.getInventory().getItem(slot) == null || player.getInventory().getItem(slot).getId() != itemId)
				return;
			if (!player.canPot())
				return;
			if (!player.getControllerManager().canPot(this))
				return;
			if (effect == null) {
