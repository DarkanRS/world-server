...
public class Alchemy {
	...
	public static boolean handleSuperheat(Player player, Item item, boolean useRunes) {
		...
		if (!player.canCastSpell())
			return false;
		if (!Magic.checkMagicAndRunes(player, 43, true, useRunes ? new RuneSet(Rune.NATURE, 1, Rune.FIRE, 4) : null))
			return false;
	...
	public static boolean handleAlchemy(Player player, Item item, boolean lowAlch, boolean useRunes) {
		...
		if (lowAlch) {
			...
			if (!Magic.checkMagicAndRunes(player, 55, true, useRunes ? new RuneSet(Rune.NATURE, 1, Rune.FIRE, 5) : null))
				return false;
		}
		player.stopAll(false, false, true);
