...
public class RunecraftingAltar  {
				...
				if (e.getPlayer().getTemporaryAttributes().get("whr") == null) {
					...
					if (selection != null) {
						if (selection.name().equals("OMNI") || selection.name().equals("ELEMENTAL")) {
							...
							return;
						} else if (selection.name().equals("SOUL")) {
	...
	public static boolean checkItems(Player player, Altar altar) {
		...
		if (player.getInventory().containsItem(altar.getTalisman(), 1) || player.getInventory().containsItem(OMNI_TALISMAN, 1))
			return true;
		if (player.getEquipment().getHatId() == altar.getTiara() || player.getEquipment().getHatId() == OMNI_TIARA)
			return true;
