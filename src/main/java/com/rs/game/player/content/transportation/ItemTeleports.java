...
import com.rs.game.player.content.world.HeroesGuild;
...
public class ItemTeleports {
	...
	private static final int[] LOWEST_AMOUNT = new int[] { 2566, 3867, 1706, 10362, 11113, 11126, 11190, 2572, 13288, 19480, 19475, 19479, 19476, 19478, 19477 };
	...
	public static void sendTeleport(Player player, Item item, int optionIndex) {
		...
		if (index < 0)
			return;
		Magic.sendTeleportSpell(player, getFirstEmote(index), -1, getFirstGFX(index), -1, 0, 0, COORDINATES[index][optionIndex], 4, true, Magic.OBJECT_TELEPORT);
	}
