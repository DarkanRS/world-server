...
public class PuroPuroImpling extends NPC {
    ...
    public static void initPuroPuroSpawns() {
        ...
        for (WorldTile tile : BABY_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(FlyingEntityHunter.FlyingEntities.BABY_IMPLING_PP.getNpcId(), tile);
        }

        for (WorldTile tile : YOUNG_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(FlyingEntityHunter.FlyingEntities.YOUNG_IMPLING_PP.getNpcId(), tile);
        }

        for (WorldTile tile : GOURMET_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(FlyingEntityHunter.FlyingEntities.GOURMET_IMPLING_PP.getNpcId(), tile);
        }

        for (WorldTile tile : EARTH_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(FlyingEntityHunter.FlyingEntities.EARTH_IMPLING_PP.getNpcId(), tile);
        }

        for (WorldTile tile : ESSENCE_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(FlyingEntityHunter.FlyingEntities.ESSENCE_IMPLING_PP.getNpcId(), tile);
        }

        for (WorldTile tile : ECLECTIC_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(FlyingEntityHunter.FlyingEntities.ECLECTIC_IMPLING_PP.getNpcId(), tile);
        }

        for (WorldTile tile : RARE_IMPLING_SPAWNS) {
            PuroPuroImpling imp = new PuroPuroImpling(getRandomRareImplingId(), tile);
        }
