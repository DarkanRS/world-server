...
public class DungeoneeringSmithing extends Action {
	...
	public static void openInterface(Player player, Item item) {
		...
		for (int i = 0;i < prods.length;i++) {
			...
			player.getPackets().sendRunScriptReverse(2263, new Object[] { prods[i].product.getName(), prods[i].product.getName(), prods[i].product.getAmount(), (934 << 16 | BASE_COMPONENTS[i] + 3) });
		}
	}
