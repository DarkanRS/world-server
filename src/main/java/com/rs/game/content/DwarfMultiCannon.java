	...
	public void tick(Player owner) {
		...
		for (int npcIndex : npcIndexes) {
			...
			if (npc.withinDistance(cannonTile, 10) && getDirectionTo(npc) == spinRot) {
				Hit hit = PlayerCombat.getRandomMaxHit(owner, npc, 0, 300, owner.getEquipment().getWeaponId(), owner.getCombatDefinitions().getAttackStyle(), PlayerCombat.isRanging(owner), true, 1.0);
