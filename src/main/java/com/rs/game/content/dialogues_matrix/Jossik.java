				...
				.addNext(() -> {
					for (int i = 0; i < player.getPrayerBook().length; i++) {
						if (player.getPrayerBook()[i] && !player.containsOneItem(PrayerBooks.BOOKS[i] + 1)) {
							player.simpleNPCDialogue(JOSSIK, HeadE.CHEERFUL, "As a matter of fact I did! I found a " + ItemDefinitions.getDefs(PrayerBooks.BOOKS[i] + 1).getName() + ", and I recognised it as yours immediately! Here you go!");
