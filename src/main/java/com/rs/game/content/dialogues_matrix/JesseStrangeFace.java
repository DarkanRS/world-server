...
public class JesseStrangeFace extends Conversation {
	public JesseStrangeFace(Player player) {
		super(player);
		addPlayer(HeadE.SKEPTICAL_HEAD_SHAKE, "Hello?");
		addSimple("Hello.", () -> {player.getPackets().sendVoice(7890);});
		...
		addSimple("It is my duty inform you that many warriors fight here, and they all succumb to defeat eventually. If that instills terror in you, " +
				"walk away now.", ()->{player.getPackets().sendVoice(7881);});
		...
		addSimple("If that is the terminolgy you would use, yes. Through the powers \"bestowed upon me by my creator, I can generate opponents for you\", " +
				"\"based on your memories of them. Men and women have fought here\", \"for generations.", ()->{player.getPackets().sendVoice(7908);});
		addPlayer(HeadE.HAPPY_TALKING, "Impressive. So you control the tower?", ()->{player.getPackets().resetSounds();});
		addSimple("The Tower is I, and I have control of the tower. I see what happens, in any corner of any floor. I am always watching.", ()->{player.getPackets().sendVoice(7909);});
		addSimple("So you believe yourself a mighty warrior?", ()->{
			player.getPackets().resetSounds();
			//player.getPackets().sendVoice(7907);//Need to find a way to reset sounds when starting options...
		});
		addOptions("Choose an option:", new Options() {
			...
			public void create() {
				option("Only the greatest warrior that ever lived!", new Dialogue()
						...
						.addSimple("Intriguing. Such belief in your own abilities...", ()->{player.getPackets().sendVoice(7906);})
						.addPlayer(HeadE.HAPPY_TALKING, "What?", ()->{player.getPackets().resetSounds();})
						.addSimple("Your confidence may have a foundation, but judgment will come in battle.", ()->{player.getPackets().sendVoice(7896);})
						.addPlayer(HeadE.HAPPY_TALKING, "You mentioned that you were created by someone, but why?", ()->{player.getPackets().resetSounds();})
						.addSimple("My purpose...must never stop...", ()->{player.getPackets().sendVoice(7902);})
						.addPlayer(HeadE.HAPPY_TALKING, "Sorry? Are you alright?", ()->{player.getPackets().resetSounds();})
						.addSimple("You must fight in the tower, warrior. Demonstrate your ability to others and learn.", ()->{player.getPackets().sendVoice(7879);})
						.addPlayer(HeadE.HAPPY_TALKING, "I'd thought that, as a guide, you'd be a little more welcoming.",()->{player.getPackets().resetSounds();})
						.addSimple("You will find I am welcoming enough.", ()->{player.getPackets().sendVoice(7911);})
						.addSimple("Now, I can offer you more guidance; or, if you overflow with confidence, you can figure out yourself. I am the " +
							...
							player.getPackets().resetSounds();
							//player.getPackets().sendVoice(7872);//Need a way to reset sounds on option
							...
							public void create() {
								...
								option("No.", new Dialogue()
										.addSimple("Your choice. Come back if you change your mind.", ()->{
											//player.getPackets().sendVoice(7878);//Need a way to reset sounds on dialogue exit
										})
								);
							}
						})
				...
				option("I'm pretty handy with a weapon.", new Dialogue()
						...
						.addSimple("Intriguing. I sense humility in you...", ()->{player.getPackets().sendVoice(7887);})
						.addPlayer(HeadE.HAPPY_TALKING, "What?", ()->{player.getPackets().resetSounds();})
				);
			}
		});
