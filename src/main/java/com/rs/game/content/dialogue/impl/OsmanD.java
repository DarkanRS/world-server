...
import com.rs.game.content.quests.handlers.princealirescue.OsmanPrinceAliRescueD;
import com.rs.game.model.entity.player.Player;
import com.rs.lib.Constants;

public class OsmanD extends Conversation {

	public OsmanD(Player player, int convoID) {
		super(player);
			...
			addOptions("Choose an option:", new Options() {
				...
				public void create() {
					...
					option("Sorcerer's Garden", new Dialogue()
							.addNext(()->{
								player.startConversation(new OsmanD(player, 0).getStart());
							}));
				}
			});
