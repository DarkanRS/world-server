...
public class ItemTeleports {
	...
	public static boolean transportationDialogue(Player player, Item item) {
		...
		if (!checkAll(player, item, index, 0, 1))
			return false;
		player.startConversation(new Transportation(), TELEPORT_NAMES[index], item.getId());
		return true;
	}
	...
	public static boolean transportationDialogue(Player player, int itemId) {
		...
		if (index == -1)
			return false;
		player.startConversation(new Transportation(), TELEPORT_NAMES[index], itemId, new Item(itemId, 1));
		return true;
	}
