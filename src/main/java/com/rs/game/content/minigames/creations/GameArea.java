	...
	public void create(Runnable callback) {
		...
		region.requestChunkBound(() -> {
			...
			World.executeAfterLoadRegion(region.getRegionId(), () -> {
				...
				World.spawnNPC(managerBlue, new WorldTile(getMinX() + Helper.BLUE_MANAGER_P[0], getMinY() + Helper.BLUE_MANAGER_P[1], 0), -1, false, true).setRandomWalk(false);
				World.spawnNPC(managerRed, new WorldTile(getMinX() + ((flags.length - 1) * 8) + Helper.RED_MANAGER_P[0], getMinY() + ((flags.length - 1) * 8) + Helper.RED_MANAGER_P[1], 0), -1, false, true).setRandomWalk(false);
				callback.run();
			});
		});
	}
