...
public final class FightPits {
					...
					if (!startedGame) {
						...
						if (minutes == 0)
							for (Player player : arena)
								player.npcDialogue(THHAAR_MEJ_KAH, HeadE.T_CALM_TALK, "FIGHT!");
						else if (minutes == 5) { // spawn tz-kih
							// spawns
							spawns = new ArrayList<>();
							for (int i = 0; i < 10; i++)
								spawns.add(new FightPitsNPC(2734, Tile.of(GAME_TELEPORTS[Utils.random(GAME_TELEPORTS.length)], 3)));
						} else if (minutes == 6)
							for (int i = 0; i < 10; i++)
								spawns.add(new TzKekPits(2736, Tile.of(GAME_TELEPORTS[Utils.random(GAME_TELEPORTS.length)], 3)));
						else if (minutes == 7)
							for (int i = 0; i < 10; i++)
			...
			if (type == 0)
				...
				if (player.hasSkull() && player.getSkullId() == 1) {// if has
					...
					if (!player.getInventory().addItem(6529, tokkul) && type == 1)
						World.addGroundItem(new Item(6529, tokkul), Tile.of(4585, 5076, 0), player, true, 180);
				}
				if (type == 1) {
					player.lock(5);
	...
	public static void setChampion() {
		...
		champion.npcDialogue(THHAAR_MEJ_KAH, HeadE.T_CALM_TALK, "Well done, you were the last person in the pit and won that fight! The next round will start soon, wait for my signal before fighting.");
	}
