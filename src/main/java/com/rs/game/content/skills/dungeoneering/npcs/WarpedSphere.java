			...
			if (hasWalkSteps()) {
				...
				for (Player player : getManager().getParty().getTeam())
					if (!player.isLocked() && player.matches(previousTile)) {
						...
						Magic.sendTeleportSpell(player, 13493, 13494, 2437, 2435, 0, 0, getManager().getTile(reference, TELEPORT_TILE[0], TELEPORT_TILE[1]), 6, false, Magic.TeleType.MAGIC, null);
					}
			}
