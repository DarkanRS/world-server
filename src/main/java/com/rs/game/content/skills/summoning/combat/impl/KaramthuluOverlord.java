...
public class KaramthuluOverlord extends FamiliarCombatScript {
	...
	public int alternateAttack(final NPC npc, final Entity target) {
		if (npc.inMeleeRange(target)) {
			...
			npc.sync(7970, 1474);
			delayHit(npc, World.sendProjectile(npc, target, 1477, new Pair<>(34, 16), 30, 5, 16).getTaskDelay(), target, Hit.magic(npc, getMaxHit(npc, npc.getCombatDefinitions().getMaxHit(), CombatStyle.MAGE, target)));
		}
