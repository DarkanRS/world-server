...
public class BonfireD extends Conversation {
	public BonfireD(Player player, GameObject object, Log[] logs) {
		...
		Dialogue makeX = addNext(new MakeXStatement(MakeXType.SELECT, -1, "Which logs do you want to add to the bonfire?", Arrays.stream(logs).mapToInt(log -> log.getLogId()).toArray()));
		for (Log log : logs)
