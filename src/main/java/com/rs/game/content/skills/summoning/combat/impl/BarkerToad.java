...
public class BarkerToad extends FamiliarCombatScript {
	...
	public static void shootCannonball(Familiar f, Entity target) {
		...
		f.sync(7703, 1401);
		delayHit(f, 1, target, PlayerCombat.getRandomMaxHit(f.getOwner(), target, 0, 300, f.getOwner().getEquipment().getWeaponId(), f.getOwner().getCombatDefinitions().getAttackStyle(), PlayerCombat.isRanging(f.getOwner()), true, 1.0)).setSource(f);
