...
public class FamiliarSpecs {
	public static int snaringWave(Player owner, Familiar familiar, Entity target, int tier) {
		...
		delayHit(familiar, World.sendProjectile(familiar, target, 2592, 41, 16, 41, 2.0, 16, 0).getTaskDelay(), target, getRangeHit(familiar, getMaxHit(familiar, (int) (familiar.getMaxHit(AttackStyle.MAGE) * (1.05 * tier)), AttackStyle.MAGE, target, 1.5)), () -> target.setNextSpotAnim(new SpotAnim(2593)));
	...
	public static int sunderingStrike(Player owner, Familiar familiar, Entity target, int tier) {
		...
		Hit hit = getMeleeHit(familiar, getMaxHit(familiar, (int) (familiar.getMaxHit(AttackStyle.MELEE) * (1.05 * tier)), AttackStyle.MELEE, target, 1.5));
