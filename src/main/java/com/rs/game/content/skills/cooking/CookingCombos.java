	...
	public static boolean handleCombos(Player player, Item used, Item usedWith) {
		...
		if (combo != null) {
			...
			if (!player.getInventory().containsItems(combo.materials))
				return true;
			player.startConversation(new CreateActionD(new Item[][] { combo.materials }, new Item[][] { combo.product }, new double[] { 0.0 }, new int[] { -1 }, new int[] { combo.req }, Constants.COOKING, 0));
			return true;
		}
		return false;
	}
