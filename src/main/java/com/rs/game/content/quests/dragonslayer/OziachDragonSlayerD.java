		...
		switch(p.getQuestManager().getStage(Quest.DRAGON_SLAYER)) {
			...
			case REPORT_TO_OZIACH -> {
				...
				if (p.getInventory().containsItem(ELVARG_HEAD)) {
					...
					addPlayer(HeadE.SAD, "I'm still working on it. I'll bring its head soon.");
					addNPC(OZIACH, HeadE.FRUSTRATED, "Ye haven't brought its head yet? Come back when ye have it.");
					return;
				}
