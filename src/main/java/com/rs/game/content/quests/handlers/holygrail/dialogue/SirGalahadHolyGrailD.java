	...
	public SirGalahadHolyGrailD(Player p) {
		...
		switch(p.getQuestManager().getStage(Quest.HOLY_GRAIL)) {
			case GO_TO_ENTRANA -> {
				...
				addNPC(NPC, HeadE.CALM_TALK, "Ah... the Grail... yes... that did fill me with wonder! Oh, that I could have stayed forever! The spear, the food, the people...");
				addPlayer(HeadE.HAPPY_TALKING, "How can I find it?");
				addNPC(NPC, HeadE.CALM_TALK, "I did not find it through looking – though admittedly I looked long and hard – eventually, it found me.");
				addPlayer(HeadE.HAPPY_TALKING, "Interesting.");
			}
			case GO_TO_MCGRUBOR, SPEAK_TO_FISHER_KING, SPEAK_TO_PERCIVAL, GIVE_AURTHUR_HOLY_GRAIL -> {
				...
				addPlayer(HeadE.HAPPY_TALKING, "I'm on a quest to find the Holy Grail!");
				addNPC(NPC, HeadE.CALM_TALK, "Ah... the Grail... yes... that did fill me with wonder! Oh, that I could have stayed forever! The spear, the food, the people...");
				addPlayer(HeadE.HAPPY_TALKING, "I seek an item from the realm of the Fisher King");
