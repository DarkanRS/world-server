...
import static com.rs.game.content.quests.handlers.familycrest.FamilyCrest.AVAN_CREST;
...
import static com.rs.game.content.quests.handlers.familycrest.FamilyCrest.GOLDSMITH_GAUNTLETS;
...
import static com.rs.game.content.quests.handlers.familycrest.FamilyCrest.PERFECT_RUBY_NECKLACE;
import static com.rs.game.content.quests.handlers.familycrest.FamilyCrest.PERFECT_RUBY_RING;
...
public class AvanFamilyCrestD extends Conversation {
	...
	public AvanFamilyCrestD(Player p) {
		super(p);
				...
				public void create() {
					...
					option("I'm looking for a man... his name is Avan Fitzharmon.", new Dialogue()
							...
							.addPlayer(HeadE.HAPPY_TALKING, "Well, I'll see what I can do", ()->{
								p.getQuestManager().setStage(Quest.FAMILY_CREST, TALK_TO_BOOT);
							})
							);
				}
			});
		...
		case GIVE_AVAN_JEWLERY -> {
			addNPC(NPC, HeadE.CALM_TALK, "So how are you doing getting me my perfect gold jewelry?");
			if(p.getInventory().containsItems(new int[]{PERFECT_RUBY_NECKLACE, PERFECT_RUBY_RING}, new int[]{1, 1})) {
				...
				addSimple("You exchange the jewlery for the family crest with Avan", ()->{
					p.getInventory().removeItems(new Item(PERFECT_RUBY_RING, 1), new Item(PERFECT_RUBY_NECKLACE));
					p.getInventory().addItem(AVAN_CREST, 1);
					p.getQuestManager().setStage(Quest.FAMILY_CREST, TALK_TO_JOHNATHAN);
				});
					...
					public void create() {
						option("That sounds good, improve them for me", new Dialogue()
								...
								.addSimple("Avan hands the gauntlets to you", () -> {
									...
									p.getInventory().addItem(GOLDSMITH_GAUNTLETS, 1);
								})
								);
	...
	public static NPCClickHandler handleDialogue = new NPCClickHandler(new Object[] { NPC }) {
		...
		public void handle(NPCClickEvent e) {
			e.getPlayer().startConversation(new AvanFamilyCrestD(e.getPlayer()).getStart());
		}
	};
}
