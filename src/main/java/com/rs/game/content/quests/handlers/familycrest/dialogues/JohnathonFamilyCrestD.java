...
import static com.rs.game.content.quests.handlers.familycrest.FamilyCrest.CHAOS_GAUNTLETS;
...
public class JohnathonFamilyCrestD extends Conversation {
	...
	private static final String USED_ANTIPOISON = "USED_ANTIPOISEN_ON_JOHN";
	public JohnathonFamilyCrestD(Player p) {
		super(p);
		switch(p.getQuestManager().getStage(Quest.FAMILY_CREST)) {
		case TALK_TO_JOHNATHAN -> {
			if(p.getQuestManager().getAttribs(Quest.FAMILY_CREST).getB(USED_ANTIPOISON)) {
				...
				addNext(()->{
					p.getQuestManager().setStage(Quest.FAMILY_CREST, KILL_CHRONOZON);
					p.startConversation(new JohnathonFamilyCrestD(p, DEMON_INQUIRY).getStart());
				});
			} else {
		...
		case KILL_CHRONOZON -> {
			if(p.getInventory().containsItem(JOHNATHAN_CREST, 1)) {
			...
			addNext(()->{p.startConversation(new JohnathonFamilyCrestD(p, DEMON_INQUIRY).getStart());});
		}
		case QUEST_COMPLETE ->  {
					...
					public void create() {
						option("That sounds good, improve them for me", new Dialogue()
								...
								.addSimple("Johnathan hands the gauntlets to you", () -> {
									...
									p.getInventory().addItem(CHAOS_GAUNTLETS, 1);
								})
								);
