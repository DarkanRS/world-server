...
public class RuneMysteries extends QuestOutline {
		...
		public DukeHoracioRuneMysteriesD(Player player) {
			...
			case 1:
				if(!player.getInventory().containsItem(15361)) {
					...
					addNext(() -> player.getInventory().addItem(new Item(15361, 1), true));
				} else {
	...
	public static NPCClickHandler handleSedridor = new NPCClickHandler(new Object[]{300}, new String[]{"Talk-to"}, e -> e.getPlayer().startConversation(new Conversation(e.getPlayer()) {
        {
                    ...
                    if (player.getQuestManager().getStage(Quest.RUNE_MYSTERIES) == 2)
                        if (!player.getInventory().containsItem(290))
                            option("About that package...", new Dialogue()
                                    ...
                                    .addPlayer(HeadE.SAD, "Yes...")
                                    .addSimple("The head wizard gives you a research package.")
                                    .addNext(() -> player.getInventory().addItem(new Item(290, 1), true)));
			...
			e.getPlayer().startConversation(new Conversation(e.getPlayer()) {
				{
									...
									option("Yes please", new Dialogue()
											.addNext(() -> {
												...
												WorldTasks.schedule(new Task() {
													@Override
													public void run() {
														e.getPlayer().tele(Tile.of(3113, 3175, 0, 0));
													}
												}, 2);
					...
					default:
						...
						addOptions("Do you want to buy some runes?", new Options() {
							...
							public void create() {
								option("Yes please", new Dialogue()
										.addNext(()-> ShopsHandler.openShop(e.getPlayer(), "auburys_rune_shop")));
								option("No thanks", () -> {
