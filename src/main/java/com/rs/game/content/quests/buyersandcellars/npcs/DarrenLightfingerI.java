...
public class DarrenLightfingerI extends Dialogue {
    ...
    public static NPCClickHandler DarrenLightfingerI  = new NPCClickHandler(new Object[] { npcid }, new String[] {"Talk-to"}, e -> {
        ...
        if (e.getPlayer().getQuestStage(Quest.BUYERS_AND_CELLARS) == 1){
            preQuest(e.getPlayer());
            return;
        }
        if (e.getPlayer().getQuestStage(Quest.BUYERS_AND_CELLARS) == 2) {
            stage2(e.getPlayer());
            return;
        }
        if (e.getPlayer().getQuestStage(Quest.BUYERS_AND_CELLARS) >= 3) {
            stage3(e.getPlayer());
        }
