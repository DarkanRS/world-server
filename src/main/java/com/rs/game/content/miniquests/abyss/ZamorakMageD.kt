...
@PluginEventHandler
class ZamorakMageD(p: Player) : Conversation(p) {
    init {
        if (p.miniquestManager.getStage(Miniquest.ENTER_THE_ABYSS) == EnterTheAbyss.NOT_STARTED) {
            addNPC(NPC, HeadE.FRUSTRATED, "Ah, you again. What was it you wanted? The Wilderness is hardly the appropriate place for a conversation now, is it?")
        }
        ...
        when (p.miniquestManager.getStage(Miniquest.ENTER_THE_ABYSS)) {
            EnterTheAbyss.SCRYING_ORB -> {
                ...
                if (!player.inventory.containsOneItem(5518, 5519)) {
                    addPlayer(HeadE.WORRIED, "Uh... No.... I kind of lost that orb thingy that you gave me...")
                    addNPC(NPC, HeadE.FRUSTRATED, "What? Incompetent fool. Take this. And do not make me regret allying myself with you.") {
                        p.inventory.addItemDrop(5519, 1)
                    }
                    create()
                }
                if (player.inventory.containsItem(5518, 1)) {
            ...
            3 -> {
                ...
                addNext {player.miniquestManager.complete(Miniquest.ENTER_THE_ABYSS) }
                create()
            }
        }
        addOptions(this, "startOptions") { ops: Options ->
            when (p.miniquestManager.getStage(Miniquest.ENTER_THE_ABYSS)) {
                1 -> {
                    if (p.isQuestComplete(Quest.RUNE_MYSTERIES)) {
                        ops.add("Where do you get your runes from?", Dialogue()
                            ...
                            .addOptions("Help the Zamorakian mage?") {
                                    helpOps: Options ->
                                helpOps.add("Yes", Dialogue()
                                    ...
                                    .addNPC(NPC, HeadE.CONFUSED, "Is that all clear, stranger?") {
                                        player.miniquestManager.setStage(Miniquest.ENTER_THE_ABYSS, EnterTheAbyss.SCRYING_ORB)
                                        player.inventory.addItemDrop(5519, 1)
                                    }
                ...
                2 -> {
                    ops.add("What am I supposed to be doing again?", Dialogue()
                            ...
                            .addGotoStage("startOptions", this@ZamorakMageD)
                    )

                    ops.add("What's in this for me, anyway?", Dialogue()
                    ...
                    ops.add("You're not going to use this for evil are you?", Dialogue()
            ...
            ops.add("All hail Zamorak!", Dialogue()
            ...
            ops.add("Nothing, thanks.", Dialogue()
