...
public class ZamorakMage extends Conversation {
	...
	public ZamorakMage(Player p) {
		...
		addOptions(this, "startOptions", ops -> {
			switch(p.getMiniquestManager().getStage(Miniquest.ENTER_THE_ABYSS)) {
				case 1 -> {
					if (p.isQuestComplete(Quest.RUNE_MYSTERIES)) {
						ops.add("Where do you get your runes from?", new Dialogue()
								...
								.addOptions("Help the Zamorakian mage?", helpOps ->
										helpOps.add("Yes", new Dialogue()
				...
				case 2 -> {
					ops.add("What am I supposed to be doing again?", new Dialogue()
					...
					ops.add("What's in this for me, anyway?", new Dialogue()
					...
					ops.add("You're not going to use this for evil are you?", new Dialogue()
			...
			ops.add("All hail Zamorak!", new Dialogue()
