...
@ServerStartupEvent
fun mapUristPlugins() {
    onNpcClick(npcid, options = arrayOf("Talk-to")) { (player, npc) ->
        ...
        when (player.getMiniquestStage(Miniquest.FROM_TINY_ACORNS)) {
            ...
            2 -> stage2(player)
            3 -> stage3(player)
            else -> postQuest(player)
        }
    }

    onNpcClick(npcid, options = arrayOf("Pickpocket")) { (player, npc) ->
        if (!player.isMiniquestStarted(Miniquest.FROM_TINY_ACORNS)) {
            player.npcDialogue(
                npcid,
                HeadE.ANGRY,
        ...
        if (player.inventory
                .containsItem(talisman)
        ) {
            player.sendMessage("You've stolen his talisman already.")
            return@onNpcClick
        }
        if (getFloorDrop(player) != null) {
            player.sendMessage("His talisman isn't in his pocket, it's on the ground nearby.")
            return@onNpcClick
        }
        if (player.inventory
                .containsItem(babyDragon) || player
                .miniquestManager.isComplete(Miniquest.FROM_TINY_ACORNS)
        ) {
            player.sendMessage("You don't need this; you've already got the dragon.")
            return@onNpcClick
        }
        player.actionManager.setAction(pickpocketUrist(npc))
    }
}
...
private fun stage1(player: Player, npc: NPC) {
    getFloorDrop(player)?.let { floorDrop ->
        player.startConversation {
            ...
            npc(npcid, HeadE.SCARED, "What thing? Oh! Thank you kindly, I'd have been sad if I'd lost that.") {
        ...
        player.startConversation {
            ...
            npc(npcid, HeadE.HAPPY_TALKING, "That I am. What can I do for you?")
            player(HeadE.CALM_TALK, "How's it coming along?")
            npc(npcid, HeadE.HAPPY_TALKING, "It's very nearly done. Just needs a bath in preserving oil to protect the mechanisms, and then a good polish.")
            player(HeadE.CALM_TALK, "Is this it here on your stall?")
            npc(npcid, HeadE.HAPPY_TALKING, "Aye, the baby red dragon there. As ordered, its scales are perfect rubies and it's capable of walking and breathing fire.")
            npc(npcid, HeadE.HAPPY_TALKING, "Could have made it fly too, given another six months or so.")
            player(HeadE.CALM_TALK, "Very nice. Well, see you later.")
            npc(npcid, HeadE.HAPPY_TALKING, "Aye, that you will.")
        }
    }
}
...
private fun stage2(player: Player) {
        if (player.inventory.containsItem(babyDragon) || player.bank.containsItem(babyDragon, 1)) {
            player.startConversation {
                player(
                    HeadE.CALM_TALK,
                    "I'm just checking up on progress for Mr. Lightfinger. You said it would be ready in a couple of days?"
                )
                npc(npcid, HeadE.HAPPY_TALKING, "Aye, it's nearly done. It's right he-")
                player(HeadE.CONFUSED, "...")
                npc(npcid, HeadE.SCARED, "By all the gods! Where's it gone? Where's it got to?")
                player(HeadE.CONFUSED, "It's not walked off, has it?")
                npc(npcid, HeadE.FRUSTRATED, "It couldn't walk that far on one turn of the spring.")
                player(
                    HeadE.CALM_TALK,
                    "I hope it turns up, then; it's due very soon. Could it have been stolen?"
                )
                npc(npcid, HeadE.FRUSTRATED, "In Varrock? With that guard watching like a hawk?")
                player(HeadE.CALM_TALK, "He doesn't seem all that hawklike.")
                npc(npcid, HeadE.SKEPTICAL, "Now that you mention it...")
                player(HeadE.CALM_TALK, "Will you be able to finish by the deadline?")
                npc(npcid, HeadE.SHAKING_HEAD, "Are you serious? Not a chance!")
                player(HeadE.CALM_TALK, "Then what will you tell Mr. Lightfinger?")
                npc(
                    npcid,
                    HeadE.VERY_FRUSTRATED,
                    "I don't see that I have much choice; I shall have to give him his money back."
                )
                player(HeadE.CALM_TALK, "That sounds fair.")
                npc(
                    npcid,
                    HeadE.HAPPY_TALKING,
                    "Since you're working for him, can you take this banker's note to him with my sincerest apologies?"
                )
                player(HeadE.CALM_TALK, "Urist hands you a banker's note. The figure on it is astronomical.") {
                ...
                npc(npcid, HeadE.CALM_TALK, "I appreciate your understanding.")
            ...
            player.startConversation {
                ...
                npc(
                    npcid, HeadE.HAPPY_TALKING, "I thought I'd lost it for a while. Lucky I found it, eh?"
    ...
    private fun stage3(player: Player) {
        player.startConversation{
            npc(npcid, HeadE.SAD_CRYING, "Ruined! Ruined! It makes me want to throw a tantrum!")
                if (!player.inventory
                        .containsItem(babyDragon) && !player.bank
                        .containsItem(babyDragon, 1)
                ) {
                    player.startConversation {
                        player(HeadE.CALM_TALK, "Oh, nice dragon.")
                        npc(
                            npcid,
                            HeadE.HAPPY_TALKING,
                            "I thought I'd lost it for a while. Lucky I found it, eh?"
                        ) {
                            player.miniquestManager.setStage(Miniquest.FROM_TINY_ACORNS, 1)
                            if (player.inventory
                                    .containsItem(bankNote)
                            ) {
                                player.inventory
                                    .deleteItem(bankNote, 1)
                ...
                if (!player.inventory
                        .containsItem(bankNote) && !player.bank
                        .containsItem(bankNote, 1)
                ) {
                    player.startConversation {
                    }
