...
public class Tirannwn {
	...
	public static ObjectClickHandler handleEnterUndergroundPass = new ObjectClickHandler(new Object[] { 4006 }, e -> {
		if (e.getPlayer().isQuestComplete(Quest.PLAGUE_CITY)) {
			PlagueCityUtils pcu = new PlagueCityUtils();
			if (!pcu.isWearingGasMask(e.getPlayer())) {
				e.getPlayer().playerDialogue(HeadE.WORRIED, "I should wear a gasmask before going through there.");
			} else {
