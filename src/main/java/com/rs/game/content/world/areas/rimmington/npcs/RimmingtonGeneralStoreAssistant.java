...
@PluginEventHandler
public class RimmingtonGeneralStoreAssistant extends Conversation {
	private static final int npcId = 531;


	public static NPCClickHandler RimmingtonGeneralStoreAssistant = new NPCClickHandler(new Object[]{npcId}, e -> {
		switch (e.getOption()) {
		//Start Conversation
		case "Talk-to" -> e.getPlayer().startConversation(new RimmingtonGeneralStoreAssistant(e.getPlayer()));
		case "Trade" -> ShopsHandler.openShop(e.getPlayer(), "rimmington_general_store");
		}
	});

	public RimmingtonGeneralStoreAssistant(Player player) {
		super(player);
