...
@PluginEventHandler
public class RimmingtonGeneralStoreShopkeeper extends Conversation {
	private static final int npcId = 530;

	public static NPCClickHandler RimmingtonGeneralStoreShopkeeper = new NPCClickHandler(new Object[]{ npcId }, e -> {
		switch (e.getOption()) {
		//Start Conversation
		case "Talk-to" -> e.getPlayer().startConversation(new RimmingtonGeneralStoreShopkeeper(e.getPlayer()));
		case "Trade" -> ShopsHandler.openShop(e.getPlayer(), "rimmington_general_store");
		}
	});

	public RimmingtonGeneralStoreShopkeeper(Player player) {
		super(player);
